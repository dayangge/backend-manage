...
@Form.create()
class givingLog extends PureComponent {
  state = {
    selectedRows: [],
    tableHeader: '今天',
    tableQuery: 'today',
  ...
  columns = [
    ...
    {
      title: '赠送前游戏币',
      dataIndex: 'oldGameCoin',
    },
    {
      title: '赠送后游戏币',
      dataIndex: 'newGameCoin',
    },
    {
      title: '地址',
      dataIndex: 'address',
      ...
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={24} sm={24}>
          ...
          <Button
            icon="check-circle"
            type="primary"
            onClick={e => this.handleQuerygivingLog(e, 'today')}
          >
            今天
          </Button>
          <Button
            icon="check-circle"
            type="primary"
            onClick={e => this.handleQuerygivingLog(e, 'yesterday')}
          >
            昨天
          </Button>
          <Button
            icon="check-circle"
            type="primary"
            onClick={e => this.handleQuerygivingLog(e, 'week')}
          >
            本周
          </Button>
          <Button
            icon="check-circle"
            type="primary"
            onClick={e => this.handleQuerygivingLog(e, 'lastWeek')}
          >
            上周
          </Button>
        </Col>
  ...
  render() {
    const {
      givingLog: { givingLogData },
      loading,
    } = this.props;
    const { selectedRows } = this.state;
    const dataSource = givingLogData.list;

    return (
      <PageHeaderWrapper title="赠送记录">
        <Card bordered={false}>
